                ...
                Ok(event) => {
                    if let Ok(json) = serde_json::to_string(&event) {
                        let event_type = match &event {
                            ...
                            ApiEvent::WorkflowRunStarted { .. } => "workflow_run_started",
                            ApiEvent::WorkflowRunUpdated { .. } => "workflow_run_updated",
                            ApiEvent::WorkflowRunCompleted { .. } => "workflow_run_completed",
                            ApiEvent::WorkflowRunFailed { .. } => "workflow_run_failed",
                        };
